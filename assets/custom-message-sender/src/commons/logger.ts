...
export class AppLogger {
  ...
  public readonly configure = (config?: AppLoggerConfig): void => {
    const logConfig = AppLogger._buildConfiguration(config);
    this._logger = this._lib(logConfig);
  };

  public readonly debug: AppLoggerLogFn = (arg: any, ...args: any[]) => {
    this._logger.debug(arg, ...args);
  };

  public readonly info: AppLoggerLogFn = (arg: any, ...args: any[]) => {
    this._logger.info(arg, ...args);
  };

  public readonly error: AppLoggerLogFn = (arg: any, ...args: any[]) => {
    this._logger.error(arg, ...args);
  };

  public readonly trace: AppLoggerLogFn = (arg: any, ...args: any[]) => {
    this._logger.trace(arg, ...args);
  };

  public readonly warn: AppLoggerLogFn = (arg: any, ...args: any[]) => {
